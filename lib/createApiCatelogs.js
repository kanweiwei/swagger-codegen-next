...
var _getRelyDtos = _interopRequireDefault(require("./getRelyDtos"));

var _transformPaths = _interopRequireDefault(require("./transformPaths"));
...
function getChildModules(childs) {
  let res = "";
  childs.forEach(c => {
    let parameters = (0, _lodashEs.groupBy)(c.parameters, "in");
    let hasQuery = "query" in parameters;
    let hasBody = "body" in parameters;
    let fnName = (0, _lodashEs.lowerCase)(c.operationId);
...
const createApiCatelogs = async json => {
  let paths = (0, _transformPaths.default)(json.paths);
  const pathNames = Object.keys(paths);
  let moduleNames = (0, _lodashEs.flow)(_lodashEs.flattenDeep, _lodashEs.uniq)(pathNames.map((pathName, i) => {
    return paths[pathName].tags;
  }));
  moduleNames = (0, _lodashEs.uniq)(moduleNames);
  console.log(moduleNames);
  const distPath = path.join(cwd, "./dist"); // 清空dist目录
  ...
  moduleNames.forEach((moduleName, i) => {
    ...
    pathNames.forEach(pathName => {
      ...
      let tag = (0, _lodashEs.first)(path.tags);
  ...
  let dtos = (0, _lodashEs.keys)(json.definitions).map(dtoName => {
    return {
      name: dtoName,
      links: []
    };
  ...
  for (let i = 0, len = pathNames.length; i < len; i++) {
    ...
    if (match) {
      ...
      names = (0, _lodashEs.uniq)(names);
      names.forEach(dn => {
        let targetDto = dtos.find(n => n.name === dn);
        let foundIndex = targetDto.links.findIndex(n => n.moduleName === (0, _lodashEs.first)(methodBody.tags));
  ...
  await writeFileAsync(path.join(cwd, "dist/dtos.json"), Buffer.from(JSON.stringify(dtos)), "utf-8"); // 创建通用的dto目录

  await mkdirAsync(path.join(cwd, "dist/dto")); // 泛型接口

  const genericDtos = dtos.filter(n => n.name.indexOf("[") > -1).map(n => n.name); // 通用接口

  const commonDtos = dtos.filter(n => n.links.length > 1 && n.name.indexOf("[") === -1).map(n => n.name);
  ...
  for (let i = 0; i < commonDtos.length; i++) {
    ...
    const relyDtos = (0, _getRelyDtos.default)(d);
    relyDtos.forEach(d => {
