...
var _getDto = require("./utils/getDto");
...
const createApiCatelogs = async (json, options) => {
  ...
  await (0, _writeFile.default)(_path.default.join(outputPath, "./dtos.json"), Buffer.from(JSON.stringify(dtoMap)), {
    encoding: "utf-8"
  }); // 创建通用的dto目录

  await mkdirAsync(_path.default.join(outputPath, ".dto"));
  let s = ""; // 泛型接口
  ...
  for (let i = 0, len = modules.length; i < len; i++) {
    ...
    if (dtos.length) {
      dtoImport += `import { ${(0, _lodash.uniq)(dtos).join(", ")} } from './dto'\n`;
    }
