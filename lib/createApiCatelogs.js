...
const createApiCatelogs = async json => {
  ...
  let moduleNames = (0, _lodash.flow)(_lodash.flattenDeep, _lodash.uniq)(pathNames.map((pathName, i) => {
    return paths[pathName].tags;
  }));
  moduleNames = (0, _lodash.uniq)(moduleNames);
  ...
  let dtos = (0, _lodash.keys)(json.definitions).map(dtoName => {
    return {
      name: dtoName,
      links: []
    };
